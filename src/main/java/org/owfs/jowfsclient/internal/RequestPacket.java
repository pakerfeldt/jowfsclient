...
public class RequestPacket extends Packet {
	 ...
	 * 
	 * @param function
	 *            the value of the message type header field.
	 * @param dataLength
	 *            value of the data length header field.
	 * @param flags
	 *            the {@link Flags} to associate with this owserver protocol
	 *            packet.
	 * @param payload
	 *            the payload to include in the packet.
	 */
	public RequestPacket(OwMessageType function, int dataLength, Flags flags,
			String payload) {
	 ...
	 *            the payload to include in the packet.
	 * @param dataToWrite
	 *            the new value to write to the owserver element (defined by the
	 *            payload).
	 */
	/* TODO: Check that datalength really should be dataToWrite.length() here */
	public RequestPacket(OwMessageType function, Flags flags, String payload,
			String dataToWrite) {
		super(0, payload.length() + 1 + dataToWrite.length(),
				function.intValue, dataToWrite.length(), flags, 0);
